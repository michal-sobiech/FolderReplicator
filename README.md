...
2. `dotnet build`
3. `cd src/FolderReplicator/bin/Debug/net8.0`
...
You can now interact with the program in a safe environment. The reference folder is `/home/app/src` and the target folder is `/home/app/dest`. Logs are in `/home/app/log.txt`. Everything from the reference folder will be replicated to the target folder. Everything that is in the target folder and not in the reference folder will be removed.
